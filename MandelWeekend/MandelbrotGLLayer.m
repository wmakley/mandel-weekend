...
@interface MandelbrotGLLayer (Private)
...
@implementation MandelbrotGLLayer
...
- (instancetype)initWithFrame:(NSRect)frame {
    self = [super init];
    if (self) {
//        [self setWantsBestResolutionOpenGLSurface:YES]; // TODO: enable retina support

        _zoomX = 0.0;
        _zoomY = 0.0;
        _zoomScale = 1.0;
        _maxIterations = [MandelbrotGLLayer defaultMaxIterations];
        _isRendering = NO;
        _texture = [[GradientTexture alloc] init];

        _baseGraphSize = [MandelbrotGLLayer defaultGraphSize];
        _baseTranslation = [MandelbrotGLLayer defaultTranslation];
    }
    return self;
}
