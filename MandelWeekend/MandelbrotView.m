...
- (void)setTranslationUniformX:(CGFloat)x Y:(CGFloat)y;
- (void)setScaleUniform:(CGFloat)scale;
- (void)setMaxIterationsUniform:(NSInteger)maxIterations;
...
//    self.sampleCount = self.renderer.multisamples;
    ...
    _zoomY = 0.0;
    _zoomScale = 1.0;
    _maxIterations = [MandelbrotView defaultMaxIterations];
    _isRendering = NO;
    _isLiveResizing = NO;
//        _texture = [[GradientTexture alloc] init];

    _baseGraphSize = [MandelbrotView defaultGraphSize];
    _baseTranslation = [MandelbrotView defaultTranslation];
}
...
//- (void)resize {
//    NSSize screenSize = [self screenSizeInPixels];
//    NSLog(@"resize %f x %f", screenSize.width, screenSize.height);
////    glViewport(0, 0, screenSize.width, screenSize.height);
//    [self setScreenSizeUniform:screenSize];
